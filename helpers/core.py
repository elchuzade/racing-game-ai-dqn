...
class Car:
    ...
    def __init__(self, x, y):
        ...
        self.width = Constants.CAR_WIDTH.value
        # height is originating from car image width
        self.height = Constants.CAR_HEIGHT.value
...
class My_car(Car):
    ...
    def move(self, direction):
        ...
        if direction == "left" and self.x > Constants.MARGIN.value + Constants.LINE_WIDTH.value/2:
            # 80 is the distance between the road lines
            self.x = self.x - Constants.CAR_HEIGHT.value
        elif direction == "right" and self.x < Constants.WIDTH.value - Constants.CAR_WIDTH.value:
            self.x = self.x + Constants.CAR_HEIGHT.value
